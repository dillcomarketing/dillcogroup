...
<header class="py-5 px-4 lg:px-8 {{ $headerClasses }}">
    <ul role="navigation" class="flex flex-wrap items-center justify-between uppercase text-sm font-semibold">
        ...
        @foreach ($menus as $menu)
            <li class="border-b-2 {{ $menu['active'] ? 'border-' . $accent : 'border-transparent'  }} hover:opacity-70 hover:border-{{ $accent }}">
                <a href="{{ $menu['link'] }}" class="block" title="{{ ucwords($menu['title']) }}">
                    {{ $menu['title'] }}
                </a>
            </li>
        @endforeach
